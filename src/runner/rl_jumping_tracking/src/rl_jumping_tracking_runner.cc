#include "rl_jumping_tracking/rl_jumping_tracking_runner.h"
...
struct RlJumpingTrackingRunner::ReferenceState {
...
class RlJumpingTrackingRunner::TrackingPolicyModel {
public:
  TrackingPolicyModel(const std::string &model_path, int obs_dim,
                      int num_actions) {
    if (!std::filesystem::exists(model_path)) {
      throw std::runtime_error("Jumping policy file does not exist: " +
                               model_path);
    }
...
RlJumpingTrackingRunner::~RlJumpingTrackingRunner() = default;

void RlJumpingTrackingRunner::SetupContext() {
  data_store_->parallel_by_classic_parser.store(false);
}

void RlJumpingTrackingRunner::TeardownContext() {}

bool RlJumpingTrackingRunner::Enter() {
  if (!param_tag_.empty()) {
    param_ =
        data::ParamManager::create<data::RlJumpingTrackingParam>(param_tag_);
  }
  if (!param_) {
    LOG(ERROR) << "[RlJumpingTrackingRunner::Enter] Failed to load parameters";
    return false;
  }
  ...
  try {
    ...
    LOG(ERROR) << "[RlJumpingTrackingRunner::Enter] " << error.what();
    return false;
  }

  LOG(INFO) << "[RlJumpingTrackingRunner::Enter] Loaded Jumping policy, "
...
void RlJumpingTrackingRunner::Run() {
  if (!policy_) {
    LOG(ERROR) << "[RlJumpingTrackingRunner::Run] Policy is not loaded";
    return;
  }
  ...
  try {
    ...
    if (iter_ < 5 || iter_ % 50 == 0) {
      LOG(INFO) << "[DEBUG-JUMPING] iter=" << iter_
    ...
    LOG(ERROR) << "[RlJumpingTrackingRunner::Run] " << error.what();
    SetRunnerState(RunnerState::kTryExit);
  }
}

TransitionState RlJumpingTrackingRunner::TryExit() {
  return TransitionState::kCompleted;
}

bool RlJumpingTrackingRunner::Exit() { return true; }

void RlJumpingTrackingRunner::End() {}

void RlJumpingTrackingRunner::InitializeJointMapping() {
  ...
  if (param_->joint_stiffness.size() != num_actions ||
    ...
    throw std::runtime_error(
        "Jumping joint parameter vector sizes must match joint_names");
  }
  ...
  for (int i = 0; i < num_actions; ++i) {
    ...
    if (!model_param_->joint_id_in_total_limb.contains(joint_name)) {
      throw std::runtime_error("Unknown SDK joint name in Jumping config: " +
                               joint_name);
    }
...
void RlJumpingTrackingRunner::InitializeObservationBuffers() {
  ...
  if (single_frame_dim != 161) {
    throw std::runtime_error(
        "Unexpected Jumping single-frame observation dim: " +
        std::to_string(single_frame_dim));
  }
}

void RlJumpingTrackingRunner::InitializeReferenceAlignment() {
  ...
  LOG(INFO) << "[RlJumpingTrackingRunner::InitializeReferenceAlignment] "
            << "reference anchor aligned to robot anchor, translation=("
            << reference_alignment_pos_.transpose() << ")";
}

void RlJumpingTrackingRunner::ResetObservationBuffers() {
...
void RlJumpingTrackingRunner::ReadCurrentState() {
...
RlJumpingTrackingRunner::ReferenceState
RlJumpingTrackingRunner::SampleReference(int time_step) {
  ReferenceState reference = policy_->RunReference(zero_observation_, time_step);
  ApplyReferenceAlignment(reference);
  return reference;
}

void RlJumpingTrackingRunner::ApplyReferenceAlignment(
...
Eigen::VectorXf
RlJumpingTrackingRunner::BuildObservation(const ReferenceState &reference) {
...
Eigen::VectorXf
RlJumpingTrackingRunner::ProcessObservationTerm(size_t term_index,
...
int RlJumpingTrackingRunner::GetObservationDim(const std::string &name) const {
  if (name == "command")
...
int RlJumpingTrackingRunner::ComputeObservationDim() const {
  int dim = 0;
  for (size_t i = 0; i < param_->observation_names.size(); ++i) {
...
void RlJumpingTrackingRunner::CalculateWarmupMotorCommand(
  ...
  if (iter_ < 5 || iter_ % 50 == 0) {
    LOG(INFO) << "[DEBUG-JUMPING] warmup iter=" << iter_
              << " alpha=" << alpha
...
void RlJumpingTrackingRunner::CalculateMotorCommand(
    const Eigen::VectorXf &action) {
  if (action.size() != param_->num_actions) {
    throw std::runtime_error("Jumping policy action size mismatch");
  }
...
void RlJumpingTrackingRunner::SendMotorCommand() {
  qd_des_ = Eigen::VectorXd::Zero(model_param_->num_total_joints);
  tau_ff_des_ = Eigen::VectorXd::Zero(model_param_->num_total_joints);
  GetMutableOutput().SetCommand(q_des_, qd_des_, joint_kp_, joint_kd_,
                                tau_ff_des_);
...
void RlJumpingTrackingRunner::AdvanceTimeStep() {
  ++iter_;
...
data::LinkInfo RlJumpingTrackingRunner::GetRobotAnchorState() const {
